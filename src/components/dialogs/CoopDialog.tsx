...
export function CoopDialog({
        ...
        return (
            <Dialog open={isOpen} onOpenChange={onClose}>
                <DialogContent
                    ...
                    className="flex items-center justify-center bg-[#ecb17a] border-[#b66e65]"
                >
    ...
    if (playerFarm.farmIndex === 0) {
        return (
            <Dialog open={isOpen} onOpenChange={onClose}>
                <DialogContent
                    showCloseButton={false}
                    className="bg-[#ecb17a] border-[#b66e65] border-8"
                >
                    <DialogHeader className="text-center">
                        <DialogTitle className="text-3xl text-[#5a4535]">
                            Establish Your Farm
                        </DialogTitle>
                        <DialogDescription>
                            You must purchase your first farm plot to begin.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="flex flex-col gap-4 py-4">
                        <Button
                            onClick={() => purchaseInitialFarm()}
                            disabled={isConfirming}
                            className="bg-[#5a4535] hover:bg-[#5a4535]/90 text-white"
                        >
                            {isConfirming ? "Confirming..." : "Purchase Farm"}
                        </Button>
                    </div>
                </DialogContent>
            </Dialog>
    ...
    const nextUpgrade = farmUpgrades.find(
        (upgrade) => upgrade.maxChickens === totalSlots + 1
    );
    ...
    const renderUpgradeButton = () => {
        ...
        if (!hasEnoughBalance) {
            return (
                <Button
                    disabled={true}
                    className="w-full bg-[#5a4535] text-white py-6 text-lg"
                >
                    Insufficient $EGG
                </Button>
            );
        ...
        if (!hasEnoughAllowance) {
            return (
                <Button
                    onClick={approveEggTokens}
                    disabled={isConfirming}
                    className="w-full bg-blue-600 hover:bg-blue-700 text-white py-6 text-lg flex items-center gap-2"
                >
                    <ShieldCheck />{" "}
                    {isConfirming ? "Approving..." : `Approve $EGG`}
                </Button>
            );
        }
        return (
            ...
            >
                {isConfirming ? "Upgrading..." : "Upgrade"}
            </Button>
        );
    };
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent
                className="border-8 sm:max-w-7xl text-[#5a4535] max-h-[calc(100vh-2rem)] overflow-y-auto"
                        ...
                        <DialogHeader className="text-center mb-4">
                            ...
                            <DialogDescription>
                                <span className="inline-flex bg-[#b66e65] p-2 text-stone-100 rounded-md font-bold">
                                    Occupied Slots: {occupiedSlotsCount} /{" "}
                                    {totalSlots}
                                </span>
                            </DialogDescription>
                        </DialogHeader>
                        ...
                        {nextUpgrade ? (
                            <div className="flex flex-col gap-4">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="flex justify-between items-center">
                                        <span className="flex items-center gap-2">
                                            <Clock size={18} /> Cooldown Time:
                                        </span>
                                        <UpgradeTimer
                                            initialSeconds={
                                                playerData.farmUpgradeCooldown
                                            }
                                        />
                                    </div>
                                </div>
                            ...
                            <div className="text-center p-8">
                                ...
                                <p>Your farm has reached the maximum level.</p>
                            </div>
                        )}
                    </div>
                </div>
