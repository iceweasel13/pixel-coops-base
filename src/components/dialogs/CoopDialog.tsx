...
// ... (formatTime ve UpgradeTimer bileşenleri aynı kalabilir) ...
...
export function CoopDialog({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent showCloseButton={false} className="bg-[#ecb17a] border-[#b66e65] border-8">
                ...
                <div className="flex flex-col gap-4 py-4">
                    <Input placeholder="Referans Adresi (Opsiyonel)" value={referral} onChange={(e) => setReferral(e.target.value)} className="bg-white/50 border-[#5a4535]" />
                    <Button onClick={() => purchaseInitialFarm(referral)} disabled={isConfirming} className="bg-[#5a4535] hover:bg-[#5a4535]/90 text-white">
  ...
  const coopSlots = Array.from({ length: 10 }).map((_, index) => {
    const id = index + 1;
    if (index < occupiedSlotsCount) { return { id, status: 'occupied' as const, chicken: { ...playerChickens[index], imageUrl: '/chickens/chicken-1.png' } }; }
  ...
  const renderUpgradeButton = () => {
    if (!nextUpgrade) return null; // Maksimum seviye ise buton gösterme
    ...
    if (!hasEnoughBalance) {
        return <Button disabled={true} className="w-full bg-[#5a4535] text-white py-6 text-lg">Yetersiz $EGG</Button>;
    }
    
    if (!hasEnoughAllowance) {
        return <Button onClick={() => approveEggTokens(costInBigInt)} disabled={isConfirming} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-6 text-lg flex items-center gap-2"> <ShieldCheck /> {isConfirming ? 'Onaylanıyor...' : `$EGG Onayla`} </Button>;
    }

    return <Button onClick={buyNewFarm} disabled={isConfirming || playerData.farmUpgradeCooldown > 0} className="w-full bg-[#5a4535] hover:bg-[#5a4535]/90 text-white py-6 text-lg"> {isConfirming ? 'Yükseltiliyor...' : 'Yükselt'} </Button>;
  };

  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent 
        className="border-8 sm:max-w-5xl text-[#5a4535]" style={{ backgroundColor: '#ecb17a', borderColor: '#b66e65' }} showCloseButton={false} >
        <DialogClose aria-label="Kapat" className="absolute right-3 top-3 z-20 size-10"> <img src="/icons/close.png" alt="Kapat" className="h-10 w-10" /> </DialogClose>
                ...
                {nextUpgrade ? (
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between items-center"><span className="flex items-center gap-2"><ChevronsUp size={18}/> Yeni Slot Sayısı:</span><span className="font-bold">{nextUpgrade.maxChickens}</span></div>
                            <div className="flex justify-between items-center"><span className="flex items-center gap-2"><Zap size={18}/> Toplam Güç Kapasitesi:</span><span className="font-bold">{nextUpgrade.totalProductionPower}</span></div>
                            <div className="flex justify-between items-center"><span className="flex items-center gap-2"><Clock size={18}/> Bekleme Süresi:</span><UpgradeTimer initialSeconds={playerData.farmUpgradeCooldown} /></div>
                        </div>
                    ...
                    <div className="text-center p-8"><p className="font-bold">Tebrikler!</p><p>Çiftliğin son seviyeye ulaştı.</p></div>
                )}
            </div>
        </div>
      </DialogContent>
